...
# ===================================================================
# 2. PAIR TRADING ENVIRONMENT (DISCRETE, 16 FEATURES, NO HARD RISK)
# ===================================================================
...
class PairTradingEnv(gym.Env):
    ...
    def __init__(self, series_x: pd.Series, series_y: pd.Series,
                 ...
                 position_scale: int = 100,  # Req: Sizing with position_scale
        ...
        # Req: Discrete Action Space (Short, Flat, Long)
    ...
    def _get_observation(self, idx: int) -> np.ndarray:
        """Build NORMALIZED observation vector (16 Features)"""
        ...
        # Normalize position by position_scale for the observation
        obs = np.array([
            ...
            float(self.position / self.position_scale), # Normalized current position
    ...
    def step(self, action: int):
        """
        Execute one trading step with DISCRETE actions.
        """
        ...
        if is_last_step:
            ...
            target_position = 0 # Force exit on last step
        else:
        ...
        if trade_occurred:
            ...
            # Estimate notional for cost calc
            ...
            if self.position != 0 and target_position == 0:
                self.trade_history.append({
                    ...
                    'holding_days': self.days_in_position, 'forced_close': is_last_step
                })
        else:
        ...
        info = {
            ...
            # Extra fields for 16-feature set (ADF/Half-Life)
...
class OperatorAgent:
    ...
    def train_on_pair(self, prices: pd.DataFrame, x: str, y: str,
        ...
        print(f"\n{'='*70}")
        print(f"Training pair: {x} - {y} (LSTM POLICY - DISCRETE V3)")
        print(f"  Timesteps (RL): {timesteps:,}")
        print(f"  Epochs: 15")
        print(f"{'='*70}")

        # Discrete Env with position_scale
        env = PairTradingEnv(
        ...
        # Req: 15 Training Epochs
        model = RecurrentPPO(
            "MlpLstmPolicy", env,
            learning_rate=0.0001, n_steps=4096, batch_size=256, n_epochs=15, gamma=0.99,
...
def run_operator_holdout(operator, holdout_prices, pairs, supervisor):
    """
    Run holdout testing with supervisor monitoring and LSTM Warm-up loop.
    """
    ...
    for pair in pairs:
        ...
        if len(aligned) < WARM_UP_STEPS + 1:
            print(f"⚠️ Insufficient data ({len(aligned)} steps) - skipping")
            continue
        ...
        # CLEAN RESET (Reset financials, keep LSTM state and Env Index)
        start_idx_for_trading = env.idx 
        env.reset() 
        env.idx = start_idx_for_trading 
        print(f"  ✓ Warm-up complete. Starting live trading from index {env.idx}.")
        # ---------------------
